...
| **Redemption outcome** | Per-channel, not all-or-nothing: one redemption (`POST /members` or `junto add-member --enroll`) walks the invite's recovered channel set and reports `granted` / `already_a_member` / `invite_already_used` / `not_founder` / `failed` for each. The burn is asymmetric: a channel refused *before* any attempt (`not_founder`, `already_a_member`) never touches the invite record and stays retryable; every other channel is consumed **before** the append runs (fails closed), so a channel that fails *after* the burn (`failed`) is already spent — a retry reports `invite_already_used` for it, not a second attempt. Re-pasting the same code retries exactly the pre-attempt refusals, never a post-burn failure. |
