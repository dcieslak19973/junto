...
| **Identity surface** | The native surface as an **authorization site**, not just a display (ADR 0036): six endpoints — `POST /invites`, `POST /devices/enroll`, `POST /devices/preview`, `POST /members`, `GET /channels/{c}/keys.json`, and a retire/revoke pair — authorizing as `WriteAuth::Human`, no member code (ADR 0021). `keys.json` publishes 16-hex fingerprints only, never a full public key. `unverified` and `unrecognized` badges now render natively as well as on the web surface. |
