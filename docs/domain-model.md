...
|---|---|---|
| **Channel** | One workspace for one *unit of inquiry* (a question / piece of work); fuses conversation + work + party + gates + record. **Repo-agnostic:** no repo is part of a channel's identity or scope — a channel may work through one repo, several, or none. Identity is a **globally unique minted id**; the human **name is a label**, bound to the id when the channel is *opened*, **not required to be unique anywhere** (ADR 0014, amended by the collapse — spec §2). |
