...
## External assessment — "harness engineering" (Ryan / OpenAI, AI Native DevCon)

> 🔎 **Reviewed 2026-06-19 — [Ryan from OpenAI, "Harness Engineering"](https://www.youtube.com/watch?v=c8bE0cj7vHY).** A practitioner talk (Codex CLI/app) coining *harness engineering*: **making "what it means to do a good job" legible as text and just-in-time surfacing it to the agent over its trajectory, so every PR adheres to a "golden thread" of acceptable quality.** It is the strongest outside articulation of this doc's loop — and it diverges from junto on three load-bearing points. Recorded here because the talk's nightly "dream over it" distillation *is* this doc's afferent nerve (`Observability` section), and its divergences sharpen junto's bets.

**Where it converges with junto (independent arrival at the same design):**

- **The loop is the same shape.** His ladder — *trash+reprompt (not durable) → write it down → empower a reviewer agent to judge every diff → static guardrails/lints/tests* — is junto's `outcome signal → proposal → eval → gate → promote` (the "Recommended first rung"). "I never want to give the same review feedback twice; make every mistake statically impossible going forward" is exactly **promote a durable policy change instead of a point-in-time fix**.
- **Capture every signal, distill it.** "Systematize capturing every review comment, every interrupt, every failed build, every prod exception — slurp it up and *dream over it every night*, point sub-agents at it to distill missing guardrails / better prompts / more autonomy." This is the **Observability-as-afferent-nerve** framing here, almost verbatim: *no observability → no evals → no self-improvement.* His "next talk" reference is the nightly-distill loop.
- **The three foundational limits = junto's scarce resources.** Human time, **attention (must sum to one)**, and the context window. Maps onto `attention.md` (the attention router) and *Driving objectives* (human-attention economy, cost). His "auto-compaction obliterates and rebuilds context → continually re-surface it" is junto's recall bridge ([ADR 0013](adr/0013-host-serves-the-read-surface-recall-via-hook.md)): *the brief carries state, not history.*
- **LLM-as-judge reviewers + separation of roles.** Reviewer agents as a CI matrix pointed at markdown "review persona" guardrail files, collaborating with the implementation agent over the PR thread — junto's **Grader/Rubric** and the coder/reviewer/bug-finder **role isolation** above.
- **Treat the agent as a teammate; demand evidence, not scrollback.** "Biased toward merge, but it must *convince* me — show the staging logs, post a repro video (ffmpeg + headless browser)." This is junto's **Member** (agents first-class) + **Artifact** (verifiable output, URI+digest, not scrollback) + **Gate**, arrived at from the opposite direction.
- **"All code is prompt."** Unify the codebase on one pattern (one observability stack, not six) to spend less model attention — corroborates junto's readability-as-substrate stance (CLAUDE.md: navigable by humans *and* agents).

**Where it diverges from junto (and why junto holds its bet):**

- **Shift *right*, not left — minimize the operator's synchronous attention.** His deliberate inversion of DevOps: push interventions as far right as possible, let guardrails *auto-discover* into context, go headless. junto's gate model is the **opposite reflex for consequential actions** — they *pass through* a gate a member ratifies. The reconciliation is [ADR 0026](adr/0026-routing-policy-resolves-to-auto-or-gate-the-autonomy-envelope.md): junto *agrees* autonomy should widen, but only as evals earn it, and adds the two invariants the talk never mentions — **no self-widening** (the harness can't widen its own envelope without a human gate) and **grade ≠ consent** (a Grader's `satisfied` is read, never *grants* autonomy). The talk's optimism about auto-discovery is exactly the trust boundary junto refuses to assume.
- **No eval-validity rigor.** He trusts LLM-as-judge + static guardrails + nightly distillation, with **no held-out evals, no Goodhart concern, no self-preference worry.** That is precisely **THE CRUX** this doc treats as the whole ballgame. junto is adversarial where the talk is sanguine: an agent (or its same-family judge) grading its own output shares its blind spots → independence must come from **model diversity / execution grounding**, not a second LLM opinion.
- **Solo-leverage, single-repo, code-only — vs. a multi-member governed substrate.** The talk is one engineer running ~3 concurrent sessions on a laptop, "writing it down" into in-repo markdown/CLAUDE.md. junto is **multi-member, multi-playbook (code is one of several), vendor-neutral by adapters**, and deliberately keeps the durable record **out of the working tree** — a provenance-bound, append-only git-refs ledger ([ADR 0001](adr/0001-ledger-is-the-durable-record.md)/[0009](adr/0009-git-refs-substrate-ndjson-per-author.md)), not files that pollute `git status`. His "write it down" has no provenance binding, no membership/identity, no cross-session shared record — the collaboration substrate is the part junto is *for*.
- **Terminal-centric human surface.** He operates through Codex CLI/app; junto is **terminal-less for humans** (constraint #2 — GUI surface), agreeing only that *agents* run shells underneath and their output is captured as Artifacts, never rendered as scrollback.

**Net:** the talk is field evidence that junto's *loop* and *afferent nerve* are converged-upon by a serious practitioner — and a clean foil for junto's three differentiators (held-out evals over LLM-judge faith, a governed gate/autonomy-envelope over reflexive shift-right, and a provenance-bound multi-member record over in-repo markdown).
...
- **The human-gate is the attention sink.** Reduce what reaches it via auto-eval + auto-filter; raise autonomy (drop the gate for low-risk, eval-trusted changes) specifically to relieve attention — but only as evals earn trust (THE CRUX trust boundary). **The mechanism for this is the autonomy envelope (ADR 0026):** a human ratifies the Routing Policy for a region of policy targets, and inside it a Grader-`satisfied` proposal auto-resolves its gate and emits a notification (release notes) instead of pausing. Two invariants keep it safe — *no self-widening* (editing the Routing Policy, i.e. widening the harness's own autonomy, **always** routes to a human gate, so the loop can't escalate itself — the harness *proposes* a wider envelope and a human gates that one meta-decision) and *grade ≠ consent* (the Grader's `satisfied` is read by the Routing Policy, it never *grants* the autonomy — anti-self-preference). This is the structural form of the trust-boundary escalation below: autonomy widens only as evals earn it, one gated meta-decision at a time.
