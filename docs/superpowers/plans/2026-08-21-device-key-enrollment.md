...
**Interfaces:**
- Produces: a member revocation cutoff, derived in `project`: for each email, if **every** grant for it is retired, the cutoff is the earliest `retired_at` among them. An entry is `unrecognized` iff its author is absent from the party (existing rule) **OR** its author has a cutoff and `entry.timestamp > cutoff`.
