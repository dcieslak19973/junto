...
- **Out of bounds, being redesigned in parallel** (steer, 2026-08-21, channel `3c38ead9-4907-4646-99b7-23b21933da35`): do not touch `Workspace`, `workspace_for` (`crates/junto/src/launch.rs:425-439`), `remember_workspace` (`:466-502`), or `Host::open_channel` (`crates/junto/src/host.rs:493-560`). The one-repo-must-be-git assumption is going away and another session owns it. No task in this plan needs them.
- **Federation depends on three deliverables here** (same steer): `POST /devices/enroll` (T8), `GET /channels/{c}/keys.json` (T6), and the `unverified`/`unrecognized` badges (T11). If anything in this plan is ever cut, cut device-management polish — never those three.
- **Keep the multi-channel walk layerable, do not optimise it for a small set.** `channels_for` returns a *set*, and redemption is a walk producing one outcome per element. Threads are about to get cheap (the collapse decision in that channel), so a later "grant across all my channels" or device-level grant must be able to replace the *source* of the set and reuse the walk without a third envelope version.
