...
**A known limit worth recording:** this is achieved indirectly, not enforced. `project_channel_standing` was deliberately left untouched — it still promotes a channel to `Standing` on *any* ratified entry, a ratified `Finding` included. The guarantee is behavioral: because `attention_for_view` and the brief's act list exclude `Finding`-kind assertions, nothing *prompts* a human to ratify one, so in the ordinary flow a findings-only channel never accumulates a ratified entry and stays `Scratch`. The escape hatch is not obscure, though: the web channel page already renders one — `entry_card`'s `verification_form` (`render.rs`) offers a ratify/park form for *any* provisional assertion, `kind` never consulted, so a provisional `Finding` carries an ordinary one-click ratify button on the human surface. A human who clicks it still promotes the channel; the guarantee is that nothing *asks* them to, not that nothing lets them. This is named here so the boundary is not mistaken for a kernel-level lock that doesn't exist.
