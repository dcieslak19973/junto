...
- **Recognition** is by an **opaque, playbook-supplied `kind` tag on `Proposal`** — *not* by parsing the human-readable `action` string (which a wording change would silently break). `EntryPayload::Proposal` gains an optional `kind: Option<String>` that the kernel **stores but never interprets** — the same pattern as `ArtifactAttached.kind` (`0020`): a playbook-supplied label, not a kernel enum. The code-PR Playbook sets `kind = Some("code-pr.open-pr")`; the executor matches that stable machine key. The field is `#[serde(skip_serializing_if = "Option::is_none")]`, so every pre-existing proposal's canonical bytes are unchanged (the same backward-compatible extension `0019` made for `frame`). Extends `0003`'s content model like `0019`/`0020` did; kernel/playbook seam intact (`0007`).
