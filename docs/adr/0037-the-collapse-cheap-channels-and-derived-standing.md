...
`crate::launch::session_workdir` resolves a session's workdir by calling `mount_with_capability`, which walks a channel's *current* subjects against the *current* mount store and returns the first `Execute`-capable mount it finds — every time it is called, on launch and again on every steer. Nothing records which mount a session actually ran in at launch. If a channel has two Execute-capable Repo subjects, mount A resolves first at launch, and A is later unmounted while B remains mounted, the *next* steer resolves to B instead — a resumed session silently moves to a different directory holding none of the session's prior work, rather than erroring or staying put. This is not hypothetical scratch-directory behavior; it applies to any channel with more than one mounted, Execute-capable subject.
