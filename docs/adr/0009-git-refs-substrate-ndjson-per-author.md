...
- **The author slug is safe, not reversible.** The email is ASCII-lowercased then percent-encoded to `[a-z0-9_-]` (dropping `.` and `@` to avoid forbidden `..` / leading-dot ref components); the real email lives in each entry's JSON, so the slug need only be unique and ref-safe. Lowercasing first matters because loose refs are *files*: on case-insensitive filesystems (NTFS, default APFS) slugs differing only in case would collide as paths while staying distinct on Linux CI. Emails differing only in case therefore share one ref — safe, since the partition exists only to avoid write contention, which the optimistic `update-ref` retry already handles.
